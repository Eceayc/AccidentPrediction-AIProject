...
class ScenarioGNNOnly(nn.Module):
    def __init__(self, gnn_hidden_dim, d_model):
        super().__init__()
...
class FrameGNN(nn.Module):
    def __init__(self, in_channels, hidden_channels):
        ...
        # GAT layers with increased capacity
        ...
        # Attention pooling with increased capacity
...
def plot_training_curves(train_losses, metrics, save_path):
    ...
    # Plot accuracy
    plt.subplot(2, 2, 2)
    plt.plot(metrics['accuracy'][::-1], label='Test Accuracy')
    ...
    plt.subplot(2, 2, 3)
    plt.plot(metrics['precision'][::-1], label='Precision')
    plt.plot(metrics['recall'][::-1], label='Recall')
    ...
    # Plot F1 score
    plt.subplot(2, 2, 4)
    plt.plot(metrics['f1'][::-1], label='F1 Score')
    plt.title('F1 Score')
    plt.xlabel('Epoch')
    plt.ylabel('Score')
    plt.legend()
...
if __name__ == '__main__':
    ...
    model = ScenarioGNNOnly(GNN_HIDDEN_DIM, TRANSFORMER_DIM).to(DEVICE)
